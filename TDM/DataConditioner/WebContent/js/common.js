...
function confirmation_dialogUpload(title,msg,w,popType,caption1,caption2,data){
	var objPop = $("#popup"), popTitle = $('#popTitle'), objOverLay = $("#popupOverlay"), objContnt = $("#popup_content");
	var popW= (w)? w : 500;
	...
	if (popType == 'Ok'){
		objContnt.append('<div class="popup_button buttonsAll8"><input type="button" value="OK" name="'+data+'" id="smgBtn_ok" onclick="hidePopup(\'popup\',\'popupOverlay\');clear_files();"/></div>');
 	}
...
function clear_files(){
	clearFields('./dataConPage1');
}

function checkfile(sender) {
    var validExts = new Array(".xlsx", ".xls", ".csv", ".txt");
    var fileExt = sender.value;
    fileExt = fileExt.substring(fileExt.lastIndexOf('.'));
    if (validExts.indexOf(fileExt) < 0) {
      confirmation_dialogUpload('Invalid file','Please select valid files like ' +validExts.toString()+ ' types.' ,400,'Ok','','',sender);
