	...
	<div class="wrapper mainAll">
		...
		<div class="container">
	      ...
	      <h2 style="color: #0098cc"><spring:message code="label.dm.dashboard"/></h2> -->
			...
			<form:form id="tdmMaskingCRDataForm" name="tdmMaskingCRDataForm"
				action="${pageContext.request.contextPath}/tdmMaskingCR">
				<c:choose>
 			...
 			%>
							...
							<table id="search_output_table" class="hoverTable"
								...
								<thead>
									<tr>
										...
										<th align="center" bgcolor="#E3EFFB" scope="col"
											class="whitefont">Description</th>
										<th align="center" bgcolor="#E3EFFB" scope="col"
											class="whitefont">Created By</th>
										<th align="center" bgcolor="#E3EFFB" scope="col"
											class="whitefont">Project Name</th>
										<th align="center" bgcolor="#E3EFFB" scope="col"
											class="whitefont">Project Phase</th>
										<th align="center" bgcolor="#E3EFFB" scope="col"
											class="whitefont">Requested Time</th>
										<th align="center" bgcolor="#E3EFFB" scope="col"
											class="whitefont">Status</th>
										<th align="center" bgcolor="#E3EFFB" scope="col"
											class="whitefont">Change Request Comment</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${tdgDtMaskRequestListDTOs}"
										var="tdgDtMaskRequestListDTOs" varStatus="status">
										<tr>											
											<c:choose>
											  <c:when test="${tdgDtMaskRequestListDTOs.status == 'Submitted'}">
											    <td align="center" title="Cancel Request" onmouseover="this.style.cursor='pointer'" onClick="cancelRequest('${tdgDtMaskRequestListDTOs.id}')"><img width="20" height="20" id="close" src="images/3.png"></td>										 
											  </c:when>
											  <c:otherwise>
											    <td align="left"></td>
											  </c:otherwise>
						...
						<ul class="grdPagination">
							<%
											...
											if (currentPage != 1) {
			   							%>
							<li><a href="tdmDtMaskDashboard?page=<%= 1 %>">First</a>
								<div>First</div></li>
							<li><a href="tdmDtMaskDashboard?page=<%= currentPage-1 %>">&lt;
									Prev</a>
								<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
							<li class="disable"><a
								href="tdmDtMaskDashboard?page=<%= 1 %>">First</a>
								<div>First</div></li>
							<li class="disable"><a
								href="tdmDtMaskDashboard?page=<%= currentPage-1 %>">&lt;
									Prev</a>
								<div>&lt; Prev</div> <%
			   								 	}
			   								}
							...
							<%
			    						...
			    						%>
							<li><a href="tdmDtMaskDashboard?page=<%= i %>"
								id="employeeLink"></a>
								<div><%= i %></div></li>
							<%
			    									}
										...
										%>
							<li><a href="tdmDtMaskDashboard?page=<%= currentPage+1 %>">Next
									&gt;</a>
								<div>Next &gt;</div></li>
							<li><a href="tdmDtMaskDashboard?page=<%= noOfPages %>">Last</a>
								<div>Last</div></li>
							<%
											} else {
											    if(noOfPages > 1) {
										%>
							<li class="disable"><a
								href="tdmDtMaskDashboard?page=<%= currentPage+1 %>">Next
									&gt;</a>
								<div>Next &gt;</div></li>
							<li class="disable"><a
								href="tdmDtMaskDashboard?page=<%= noOfPages %>">Last</a>
								<div>Last</div></li>
							<%
											    }
											}
						...
						<table style="width: 100%; border: 0">
							<tbody>
								<tr>
									<th scope="col"><input type="submit" name="export"
...
window.location.hash = "myid";
   function getRequestDtls(reqId){
	   document.location.href="./getDataMaskRequestDtls?reqId="+reqId+"&edit=false";
   }
