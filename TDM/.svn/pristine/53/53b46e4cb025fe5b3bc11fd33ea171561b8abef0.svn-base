			...
			<div class="quicklink-container">
				...
				<div class="quicklink-list">
					<ul>
						...
						<li><a href="#"><label>Refresh</label></a></li>
						<!-- <li><a TARGET="_NEW" href="./policySumRefresh" class="refresh"><label>Refresh</label></a></li> -->
					</ul>
				</div>
			</div>
		</section>
...
/* $('.refresh').click(function (event){ 
    ...
    $.ajax({
       url: './policySumRefresh',
       success: function(response) {
    	   console.log(response);
    	   if(response.val() == 'true'){
    		   alert('Refresh Successfully completed');
    	   }else{
    		   alert('Refresh failed ' + response);
    	   }
       }
    })
