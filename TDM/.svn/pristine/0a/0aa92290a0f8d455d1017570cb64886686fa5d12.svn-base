...
public class TDMProviderSearchMapperImpl implements TDMProviderSearchMapper
{
	...
	@Override
	public List<POLOSearchResultDTO> convertZurichDosToPoloDTO(List<ZurichMasterDO> zurichDos)
			throws ServiceException
	{
		try
		{
			List<POLOSearchResultDTO> poloSearchRsltDTos = null;
			if (zurichDos != null)
			{
				poloSearchRsltDTos = new ArrayList<POLOSearchResultDTO>();
				for (ZurichMasterDO zmDo : zurichDos)
				{
					POLOSearchResultDTO psrDTO = new POLOSearchResultDTO();
					psrDTO.setAddress1(zmDo.getAdd1());
					psrDTO.setAddress2(zmDo.getAdd2());
					psrDTO.setAddress3(zmDo.getAdd3());
					psrDTO.setCity(zmDo.getCity());
					psrDTO.setCountry(zmDo.getCountry());
					psrDTO.setFirstName(zmDo.getFirstName());
					psrDTO.setLastName(zmDo.getLastName());
					psrDTO.setPolicyHolderId(zmDo.getPolicyHistoryId());
					psrDTO.setPolicyId(zmDo.getPolicyId());
					psrDTO.setPostalCode(zmDo.getPostalCode());
					psrDTO.setInsPolId(zmDo.getInsPolId());
					psrDTO.setRegion(zmDo.getPolicyHolderRegion());

					poloSearchRsltDTos.add(psrDTO);
				}
			}
			return poloSearchRsltDTos;
		}
		catch (NullPointerException nullPointerEx)
		{
			logger.error(MessageConstant.TDM_FTD_MAPPER + MessageConstant.TDM_FTD_MP_NS_DO_TO_DTO
	...
	@Override
	public List<ZurichReservationDO> convertPoloDTosToZurichReservationDO(POLOSearchDTO poloDTO)
			throws ServiceException
	{
		try
		{
			List<ZurichReservationDO> zurichResDos = null;
			List<POLOSearchResultDTO> poloDTos = null;
			if (poloDTO != null && poloDTO.getPoloSearchResultList() != null)
			{
				zurichResDos = new ArrayList<ZurichReservationDO>();
				poloDTos = new ArrayList<POLOSearchResultDTO>();
				for (POLOSearchResultDTO poloResultDTo : poloDTO.getPoloSearchResultList())
				{
					if (poloResultDTo.getReservedYN() != null)
					{
						ZurichReservationDO zrDo = new ZurichReservationDO();
						zrDo.setCity(poloResultDTo.getCity());
						zrDo.setCountry(poloResultDTo.getCountry());
						zrDo.setFirstName(poloResultDTo.getFirstName());
						zrDo.setLastName(poloResultDTo.getLastName());
						zrDo.setInsPolId(poloResultDTo.getInsPolId());
						zrDo.setPolicyId(poloResultDTo.getPolicyHolderId());
						zrDo.setPolicyHolderRegion(poloResultDTo.getRegion());
						zrDo.setPostalCode(poloResultDTo.getPostalCode());
						zrDo.setAdd1(poloResultDTo.getAddress1());
						zrDo.setAdd2(poloResultDTo.getAddress2());
						zrDo.setAdd3(poloResultDTo.getAddress3());
						zrDo.setLocked("Y");
						zurichResDos.add(zrDo);
					}
					else
					{
						poloDTos.add(poloResultDTo);
					}
				}
				poloDTO.setPoloSearchResultList(poloDTos);
			}
	...
	@Override
	public List<PoloReservationDTO> convertZResvrationDosToPReservationDTO(
			List<ZurichReservationDO> reservDos) throws ServiceException
	{
		try
		{
			List<PoloReservationDTO> poloDTos = null;
			if (reservDos != null && reservDos.size() > 0)
			{
				poloDTos = new ArrayList<PoloReservationDTO>();
				for (ZurichReservationDO resrvationDo : reservDos)
				{
					PoloReservationDTO resvDTo = new PoloReservationDTO();
					resvDTo.setCity(resrvationDo.getCity());
					resvDTo.setCountry(resrvationDo.getCountry());
					resvDTo.setFirstName(resrvationDo.getFirstName());
					resvDTo.setLastName(resrvationDo.getLastName());
					resvDTo.setInsPolId(resrvationDo.getInsPolId());
					resvDTo.setPolicyHolderRegion(resrvationDo.getPolicyHolderRegion());
					resvDTo.setPostalCode(resrvationDo.getPostalCode());
					resvDTo.setPolicyId(resrvationDo.getPolicyId());
					resvDTo.setAdd1(resrvationDo.getAdd1());
					resvDTo.setAdd2(resrvationDo.getAdd2());
					resvDTo.setAdd3(resrvationDo.getAdd3());

					poloDTos.add(resvDTo);
				}
			}
	...
	@Override
	public List<ZurichReservationDO> convertPOloReservationDTOtoDOs(POLOSearchDTO poloDTO)
			throws ServiceException
	{
		try
		{
			List<ZurichReservationDO> zurichResDos = null;
			List<PoloReservationDTO> poloDTos = null;
			if (poloDTO != null && poloDTO.getPoloResrvationDTOList() != null)
			{
				zurichResDos = new ArrayList<ZurichReservationDO>();
				poloDTos = new ArrayList<PoloReservationDTO>();
				for (PoloReservationDTO poloResultDTo : poloDTO.getPoloResrvationDTOList())
				{
					if (poloResultDTo.getUnreserveYN() != null)
					{
						ZurichReservationDO zrDo = new ZurichReservationDO();
						zrDo.setCity(poloResultDTo.getCity());
						zrDo.setCountry(poloResultDTo.getCountry());
						...
						zrDo.setLastName(poloResultDTo.getLastName());
						zrDo.setInsPolId(poloResultDTo.getInsPolId());
						zrDo.setPolicyId(poloResultDTo.getPolicyId());
						zrDo.setPolicyHolderRegion(poloResultDTo.getPolicyHolderRegion());
						zrDo.setPostalCode(poloResultDTo.getPostalCode());
						zrDo.setAdd1(poloResultDTo.getAdd1());
						zrDo.setAdd2(poloResultDTo.getAdd2());
						zrDo.setAdd3(poloResultDTo.getAdd3());
						zrDo.setLocked("N");
						zurichResDos.add(zrDo);
					}
					else
				...
				poloDTO.setPoloResrvationDTOList(poloDTos);
			}
