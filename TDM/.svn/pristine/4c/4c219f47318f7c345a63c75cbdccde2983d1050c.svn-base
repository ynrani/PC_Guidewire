		...
		TDMProviserSearchService
{
	...
	@Override
	public Boolean unReserveResrvedRecords(Object obj, String userId) throws ServiceException {
		logger.info(MessageConstant.TDM_FTD_SERVICE
				+ MessageConstant.TDM_UNRESERVE_RESERVED_RECORDS
				+ MessageConstant.LOG_INFO_PARAMS_NO);
		try {
			List<TdmReservationDO> reservationDOList = null;
			if (obj instanceof TDMProvSearchDTO) {
				TDMProvSearchDTO tdmProvSearchDTO = (TDMProvSearchDTO) obj;
				if (tdmProvSearchDTO.gettDMProvSearchResultListDTOs() != null
						&& 0 < tdmProvSearchDTO.gettDMProvSearchResultListDTOs().size()) {
					reservationDOList = new ArrayList<TdmReservationDO>();
					for (TDMProvSearchResultListDTO tdmPSRLDTO : tdmProvSearchDTO
							.gettDMProvSearchResultListDTOs()) {
						if (tdmPSRLDTO.getUnReservedYN() != null) {
							TdmReservationDO tdmReservationDO = new TdmReservationDO();
							tdmReservationDO.setRecordId(tdmPSRLDTO.getRecordId());
							tdmReservationDO.setLocked("N");
							tdmReservationDO.setUnreserve("Y");
							tdmReservationDO.setReserveDataType(AppConstant.PROV);
							tdmReservationDO.setTestCaseId(tdmPSRLDTO.getTestCaseId());
							tdmReservationDO.setTestCaseName(tdmPSRLDTO.getTestCaseName());
							tdmReservationDO.setReserveDataId(String.valueOf(tdmPSRLDTO
									.getProviderId()));
							tdmReservationDO.setUnlockTime(new Timestamp(new Date().getTime()));

							reservationDOList.add(tdmReservationDO);
						}
					}
				}
			} else if (obj instanceof TDMSubscSearchDTO) {
				TDMSubscSearchDTO tdmSubscSearchDTO = (TDMSubscSearchDTO) obj;
				if (tdmSubscSearchDTO.gettDMSubscSearchResultListDTOs() != null
						&& 0 < tdmSubscSearchDTO.gettDMSubscSearchResultListDTOs().size()) {
					reservationDOList = new ArrayList<TdmReservationDO>();
					for (TDMSubscSearchResultListDTO tdmSubscrDTO : tdmSubscSearchDTO
							.gettDMSubscSearchResultListDTOs()) {
						if (tdmSubscrDTO.getUnReservedYN() != null) {
							TdmReservationDO tdmReservationDO = new TdmReservationDO();
							tdmReservationDO.setRecordId(tdmSubscrDTO.getRecordId());
							tdmReservationDO.setLocked("N");
							tdmReservationDO.setUnreserve("Y");
							tdmReservationDO.setReserveDataType(AppConstant.SUB);
							tdmReservationDO.setTestCaseId(tdmSubscrDTO.getTestCaseId());
							tdmReservationDO.setTestCaseName(tdmSubscrDTO.getTestCaseName());
							tdmReservationDO.setReserveDataId(String.valueOf(tdmSubscrDTO
									.getSubscriberId()));
							tdmReservationDO.setUnlockTime(new Timestamp(new Date().getTime()));
			...
			} else if (obj instanceof TDMClaimSearchDTO) {
				TDMClaimSearchDTO tdmClaimSearchDTO = (TDMClaimSearchDTO) obj;
				if (tdmClaimSearchDTO.gettDMClaimSearchResultListDTOs() != null
						&& 0 < tdmClaimSearchDTO.gettDMClaimSearchResultListDTOs().size()) {
					reservationDOList = new ArrayList<TdmReservationDO>();
					for (TDMClaimSearchResultListDTO tdmClaimDTO : tdmClaimSearchDTO
							.gettDMClaimSearchResultListDTOs()) {
						if (tdmClaimDTO.getUnReservedYN() != null) {
							TdmReservationDO tdmReservationDO = new TdmReservationDO();
							tdmReservationDO.setRecordId(tdmClaimDTO.getRecordId());
							tdmReservationDO.setLocked("N");
							tdmReservationDO.setUnreserve("Y");
							tdmReservationDO.setReserveDataType(AppConstant.CLM);
							tdmReservationDO.setTestCaseId(tdmClaimDTO.getTestCaseId());
							tdmReservationDO.setTestCaseName(tdmClaimDTO.getTestCaseName());
							tdmReservationDO.setReserveDataId(String.valueOf(tdmClaimDTO
									.getClaimId()));
							tdmReservationDO.setUnlockTime(new Timestamp(new Date().getTime()));
			...
			EntityManager managerUser = openUserEntityManager();
			tDMProviserSearchDAO.unReserveResrvedRecords(reservationDOList, userId, managerUser);
			closeUserEntityManager(managerUser);
			logger.info(MessageConstant.TDM_FTD_SERVICE
					+ MessageConstant.TDM_UNRESERVE_RESERVED_RECORDS
					+ MessageConstant.LOG_INFO_RETURN);
