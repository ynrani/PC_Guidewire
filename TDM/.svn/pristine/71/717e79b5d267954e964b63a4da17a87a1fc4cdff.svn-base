		...
		TDMProviserSearchService
{
	...
	{
		EntityManager managerPolo = openPoloEntityManager();
		int offSet = 0;
		try
		{
			int recordsperpage = Integer.parseInt(psDTO.getSearchRecordsNo());
			List<ZurichMasterDO> zurichDoList = tDMProviserSearchDAO
					.getPoloNonStandardSearchrecords(psDTO, offSet, recordsperpage, managerPolo);
			List<POLOSearchResultDTO> poloResultDTos = tDMProviderSearchMapper
					.convertZurichDosToPoloDTO(zurichDoList);

			if (poloResultDTos != null && poloResultDTos.size() > 0)
				psDTO.setPoloSearchResultList(poloResultDTos);
			return psDTO;
		}
		catch (NullPointerException nullPointerEx)
		{
		...
		{
			closePoloEntityManager(managerPolo);
		}
	}
	...
	public int reservePoloRecords(POLOSearchDTO poloDTO, String userID) throws ServiceException
	{
		...
		{
			int count = 0;
			List<ZurichReservationDO> reservationDO = tDMProviderSearchMapper
					.convertPoloDTosToZurichReservationDO(poloDTO);
			if (reservationDO != null)
			{
				count = reservationDO.size();
			}
			tDMProviserSearchDAO.resrervePoloRecords(reservationDO, poloDTO.getTestCaseId(),
					poloDTO.getTestCaseName(), userID, managerPOlo);
			return count;
		}
		catch (Exception otherEx)
		{
			...
			throws ServiceException
	{
		...
		try
		{
			int count = 0;
			List<ZurichReservationDO> reservationDO = tDMProviderSearchMapper
					.convertPOloReservationDTOtoDOs(poloDTO);
			if (reservationDO != null)
			{
				count = reservationDO.size();
			}
			tDMProviserSearchDAO.unReservePoloResrvedRecords(reservationDO, userName, managerPOlo);
			return count;
		}
		catch (Exception otherEx)
		{
	...
	public List<PoloReservationDTO> getReservedRecords(String username) throws ServiceException
	{
		...
		try
		{
			List<ZurichReservationDO> reservationList = tDMProviserSearchDAO.getPoloResrvedRecords(
					username, managerPOlo);

			List<PoloReservationDTO> reservationDTO = tDMProviderSearchMapper
					.convertZResvrationDosToPReservationDTO(reservationList);
