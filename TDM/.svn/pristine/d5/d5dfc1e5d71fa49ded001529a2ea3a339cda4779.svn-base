...
function tdmReqCreate(){
	var formObj1 = $("#tdmRequestForm");
	var buttonObj = formObj1.find('.btn-cell');
   //form validation rules
	formObj1.validate({
		 errorClass: "my-error-class",
	     ...
	     submitHandler: function(formObj1) {
	    	 $.ajax({
	    			type:'POST',
	    				url: './tdmRequest',
	    				dataType: 'html',	
	    	 ...
	    	 //confirmation_dialogCreReq('Submit','Are you sure do you want to submit ?',400,'YesNo','Yes','No',formObj1);
	   		},
			invalidHandler: function(form, validator){
...
function submitPopCreateReq(value){
	 var values = value.split('?');
	 var url = values[0];
	 var form = values[1];
	 disablePage();
