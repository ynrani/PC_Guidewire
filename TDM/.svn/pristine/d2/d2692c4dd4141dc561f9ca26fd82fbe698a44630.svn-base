...
public class TDMMaskingController
{
	...
	@ResponseBody
	public String tdmDataSubsettingPost(
			@RequestParam(value = AppConstant.SUBMIT, required = false) String submit,
			@ModelAttribute(AppConstant.TGD_DT_MASK_DTO) TdgDataMaskingDTO tdgDataMaskingDTO,
			ModelMap model, HttpServletRequest request, HttpServletResponse response)
	{
			...
			HttpServletRequest request, HttpServletResponse response) throws BaseException
	{
		...
		{
			logger.info(MessageConstant.TDM_DT_MSK_CONTROLLER
					+ MessageConstant.TDM_CANCEL_ONBRD_REQ + MessageConstant.LOG_INFO_PARAMS_YES);
			boolean isCancelled = tdmDataMaskingService.cancelMaskingRequest(reqId);
			if (isCancelled)
			{
				logger.info(MessageConstant.TDM_DT_MSK_CONTROLLER
						+ MessageConstant.TDM_CANCEL_ONBRD_REQ + MessageConstant.LOG_INFO_RETURN
						+ isCancelled);
		...
		{
			logger.error(MessageConstant.TDM_DT_MSK_CONTROLLER
					+ MessageConstant.TDM_CANCEL_ONBRD_REQ);
