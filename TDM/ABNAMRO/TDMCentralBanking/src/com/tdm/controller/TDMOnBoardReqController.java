...
public class TDMOnBoardReqController
{
	...
	public String tdmGetOnboardReq(
			...
			@ModelAttribute(AppConstant.TDM_ONBOARDING_REQ_DTO) TdmOnBoardReqDTO tdmOnboardReqDTO,
			ModelMap model, HttpServletRequest request, HttpServletResponse response,
			Principal principal) throws BaseException {
		logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
				+ MessageConstant.TDM_ONBOARDING_REQ_GET + MessageConstant.LOG_INFO_REQ_ID + reqId);
		...
		try {
			if (StringUtils.isNotEmpty(reqId)) {
				...
				if (null != (String) request.getSession().getAttribute(AppConstant.SESSION_UID)) {
					...
					tdmOnboardReqDTO = tdmDataMaskingService.getUserDetails((String) request
							.getSession().getAttribute(AppConstant.SESSION_UID), tdmOnboardReqDTO);
					if (null != (String) request.getSession()
			...
			logger.error(MessageConstant.TDM_ONBOARDING_REQ_CTLR
					+ MessageConstant.TDM_ONBOARDING_REQ_GET + MessageConstant.LOG_ERROR_EXCEPTION
					+ baseEx);
		...
		model.addAttribute(AppConstant.TDM_ONBOARDING_REQ_DTO, tdmOnboardReqDTO);
		logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
				+ MessageConstant.TDM_ONBOARDING_REQ_GET + MessageConstant.LOG_INFO_RETURN);
		return AppConstant.TDM_ONBOARD_REQ_VIEW;
	...
	public String tdmPostOnboardReq(
			...
			Principal principal) throws BaseException {
		logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
				+ MessageConstant.TDM_ONBOARDING_REQ_POST + MessageConstant.LOG_INFO_PARAMS_NO);
		boolean chngeReqYN = false;
		try {
			...
			model.addAttribute(AppConstant.CHANGE_YN, chngeReqYN);
			logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
					+ MessageConstant.TDM_ONBOARDING_REQ_POST + MessageConstant.LOG_INFO_RETURN);
			return tdmOnboardReqDTO.getOnboardReqId();
		} catch (BaseException baseEx) {
			logger.error(MessageConstant.TDM_ONBOARDING_REQ_CTLR
					+ MessageConstant.TDM_ONBOARDING_REQ_POST + MessageConstant.LOG_ERROR_EXCEPTION
					+ baseEx);
			if (null != baseEx.getErrorCode() || baseEx.getErrorCode().equalsIgnoreCase("null")) {
				// String exceptionMsg = passcodes and get msg from properties file by passing key
				// as
	...
	public String tdmDtMaskDashboard(
		...
		logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
				+ MessageConstant.TDM_ONBOARDING_DASHBOARD + MessageConstant.LOG_INFO_PARAMS_SEPC
				+ page);
		...
		try {
			...
			totalRecords = tdmDataMaskingService.getReservedRecordsCountOnBoard((String) request
					.getSession().getAttribute(AppConstant.SESSION_UID), type);
			...
			logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
					+ MessageConstant.TDM_ONBOARDING_DASHBOARD + MessageConstant.LOG_INFO_RETURN);
			return AppConstant.TDM_ONBOARD_REQ_DASH_VIEW;

		} catch (BaseException baseEx) {
			logger.error(MessageConstant.TDM_ONBOARDING_REQ_CTLR
					+ MessageConstant.TDM_ONBOARDING_DASHBOARD
	...
	public String tdmDtMaskDashboardCR(
		...
		try {
			...
			model.addAttribute(AppConstant.TDM_DATA_MASKING_REQ_DTOS, tdgDtMaskRequestListDTOs);
			logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
					+ MessageConstant.TDM_MSK_DASHBOARD_CR + MessageConstant.LOG_INFO_RETURN);
			return AppConstant.TDM_ONBOARD_REQ_DASH_CR_VIEW;
		} catch (BaseException baseEx) {
			logger.error(MessageConstant.TDM_ONBOARDING_REQ_CTLR
					+ MessageConstant.TDM_MSK_DASHBOARD_CR + MessageConstant.LOG_ERROR_EXCEPTION
					+ baseEx);
			if (null != baseEx.getErrorCode() || baseEx.getErrorCode().equalsIgnoreCase("null")) {
				// String exceptionMsg = passcodes and get msg from properties file by passing key
				// as
	...
	public ModelAndView tdmOnBoardingExportFR(ModelMap model, HttpServletRequest request,
		...
		logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
				+ MessageConstant.TDM_ONBOARDING_DASHBOARD_FR + MessageConstant.LOG_INFO_EXPORT);
		return new ModelAndView(AppConstant.TDM_DATA_MASKING_DASH_BOARD_XLS,
				AppConstant.TDM_DATA_MASKING_DTOS, tdgDtMaskRequestListDTOfr);
	}
	...
	public ModelAndView tdmOnBoardingExportCR(ModelMap model, HttpServletRequest request,
			HttpServletResponse response) throws BaseException {
		logger.info(MessageConstant.TDM_ONBOARDING_REQ_CTLR
				+ MessageConstant.TDM_ONBOARDING_DASHBOARD_CR + MessageConstant.LOG_INFO_EXPORT);
		return new ModelAndView(AppConstant.TDM_DATA_MASKING_DASH_BOARD_XLS,
				AppConstant.TDM_DATA_MASKING_DTOS, tdgDtMaskRequestListDTOcr);
	}
}
