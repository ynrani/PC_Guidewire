		...
		function(){
			...
			if (txtbx_arrayLen == txtbx_countr){
				...
				if(noOfRecAllowed != noOfAddedRows){
					...
					confirmation_dialog('Alert','Only '+noOfRecAllowed+' rows are allowed','500','Ok');
				}
			}
			else{
				if(nested == 'nested') confirmation_dialogWithinPop(messages.promtMsg.addRow_msg.title,messages.promtMsg.addRow_msg.bodytext,'500','Ok');
				else confirmation_dialog(messages.promtMsg.addRow_msg.title,messages.promtMsg.addRow_msg.bodytext,'500','Ok');
			...
			$('tbody.'+targetContnr).delegate('.'+delRow,'click',function(){
				...
				if(nested == 'nested'){
					confirmation_dialogWithinPop(messages.promtMsg.txt_delete.title,messages.promtMsg.txt_delete.bodytext,'500','YesNo');
					confmWithinPop_deleteItem(objDelete);
				}
				else{
					confirmation_dialog(messages.promtMsg.txt_delete.title,messages.promtMsg.txt_delete.bodytext,'500','YesNo');
			...
			$.ajax({
				...
				success:function(serverResp)
				{
					...
					if ((serverResp == 'true' || serverResp == true) || (serverRespSplit[1] && serverRespSplit[0]== 'true'))
					{
						...
						if(nested=="nested"){
							confirmation_dialogWithinPop('Alert',"Data deleted successfully.",'500','Ok');
						}
						else{
							confirmation_dialog('Alert',"Data deleted successfully.",'500','Ok');
							$('#smgBtn_ok').bind('click',function(){
								hidePopup('popup','popupOverlay');
							});
						}
					...
					else
					{
						if(curRowId=='0000'){
							...
							if(nested=="nested") {
								confirmation_dialogWithinPop('Error',"Data deletion failed.",'500','Ok');
							}
							else{
								confirmation_dialog('Error',"Data deletion failed.",'500','Ok');
							}
							$('#smgBtn_ok').bind('click',function(){
								hidePopup('popup','popupOverlay');
							});
				...
				{
					if(nested=="nested") {
						confirmation_dialogWithinPop('Error',"Data deletion failed. System Error occurred. Please re-try after some time.",'500','Ok');
					}
					else{
						confirmation_dialog('Error',"Data deletion failed. System Error occurred. Please re-try after some time.",'500','Ok');
					}
					$('#smgBtn_ok').bind('click',function(){
						hidePopup('popup','popupOverlay');
					});
...
function confirmation_dialog(title,msg,w,popType,caption1,caption2){
	...
	if (popType == 'OkCancel'){
		...
		objContnt.append('<div class="ynpopup_button"><input type="button" value="'+yesCaption+'" id="btn_action" class="mRght5" /><input type="button" value="'+noCaption+'" onclick="hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
	else if (popType == 'Ok'){
		objContnt.append('<div class="popup_button"><input type="button" class="btn-popup" value="OK" id="smgBtn_ok" onclick="hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
...
function confirmation_dialogMaskingReq(title,msg,w,popType,caption1,caption2){
	var objPop = $("#popup"), popTitle = $('#popTitle'), objOverLay = $("#popupOverlay"), objContnt = $("#popup_content");
	var popW= (w)? w : 500;
	...
	if (popType == 'OkCancel'){
		...
		objContnt.append('<div class="popup_button"><input type="button" class="btn-popup" value="OK" id="smgBtn_ok" onclick="hidePopup(\'popup\',\'popupOverlay\');redirctMRboard();"/></div>');
	}
...
function confirmation_dialogOnBoardReq(title,msg,w,popType,caption1,caption2){
	var objPop = $("#popup"), popTitle = $('#popTitle'), objOverLay = $("#popupOverlay"), objContnt = $("#popup_content");
	var popW= (w)? w : 500;
	...
	if (popType == 'OkCancel'){
		...
		objContnt.append('<div class="popup_button"><input type="button" class="btn-popup" value="OK" id="smgBtn_ok" onclick="hidePopup(\'popup\',\'popupOverlay\');redirctOnboard();"/></div>');
	}
...
function confirmation_dialog_forward(url,title,msg,w,popType,caption1,caption2){
	var objPop = $("#popup"), popTitle = $('#popTitle'), objOverLay = $("#popupOverlay"), objContnt = $("#popup_content");
	var popW= (w)? w : 500;
	...
	if (popType == 'OkCancel'){
		...
		objContnt.append('<div class="popup_button"><input type="button" class="btn-popup" value="OK" id="'+url+'" onclick="forwardRequest(this.id);hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
...
function redirctMRboard(){
	window.location= "./tdmDtMaskDashboard";
}

function redirctOnboard(){
	window.location= "./tdmOnBoardingDashBoard";
}
...
function confirmation_dialogYN(title,msg,w,popType,caption1,caption2){
	...
	if (popType == 'YesNo'){
		...
		objContnt.append('<div class="ynpopup_button"><input type="button" value="'+yesCaption+'" id="btn_action" class="mRght5" onclick="submitPop();"/><input type="button" value="'+noCaption+'" onclick="hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
...
function confirmation_dialogYNCancelonBoardReq(reqId,title,msg,w,popType,caption1,caption2){
	var objPop = $("#popup"), popTitle = $('#popTitle'), objOverLay = $("#popupOverlay"), objContnt = $("#popup_content");
	var popW= (w)? w : 500;
	objContnt.html('');
	popTitle.html(title);
	...
	if (popType == 'YesNo'){
		...
		objContnt.append('<div class="ynpopup_button"><label class="lable-title" align="left">Comments</label><span>*</span> <textarea align="left" id="text_MR"  name="text_MR"></textarea><input type="button" value="'+yesCaption+'" id="'+reqId+'" class="btn-popup" onclick="submitCancelonBoardReq(this.id);"/><input type="button" class="btn-popup" value="'+noCaption+'" onclick="hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
...
function confirmation_dialogYNCancelMR(reqId,title,msg,w,popType,caption1,caption2){
	var objPop = $("#popup"), popTitle = $('#popTitle'), objOverLay = $("#popupOverlay"), objContnt = $("#popup_content");
	var popW= (w)? w : 500;
	objContnt.html('');
	popTitle.html(title);
	...
	if (popType == 'YesNo'){
		...
		objContnt.append('<div class="ynpopup_button">'+
                '<label class="lable-title" align="left">Comments</label><span>*</span> <textarea align="left" id="text_MR"  name="text_MR"></textarea><input type="button" value="'+yesCaption+'" id="'+reqId+'" class="btn-popup" onclick="submitCancelMR(this.id);"/><input type="button" class="btn-popup" value="'+noCaption+'" onclick="hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
...
function confirmation_dialogYNMR(title,msg,w,popType,caption1,caption2){
	...
	if (popType == 'YesNo'){
		...
		objContnt.append('<div class="ynpopup_button"><input type="button" value="'+yesCaption+'" id="btn_action" class="mRght5" onclick="submitPopMr();"/><input type="button" value="'+noCaption+'" onclick="hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
...
function confirmation_dialogYNCNCL(title,msg,w,popType,caption1,caption2){
	var objPop = $("#popup"), popTitle = $('#popTitle'), objOverLay = $("#popupOverlay"), objContnt = $("#popup_content");
	var popW= (w)? w : 500;
	...
	if (popType == 'YesNo'){
		...
		objContnt.append('<div class="ynpopup_button"><input type="button" value="'+yesCaption+'" id="btn_action" class="mRght5" onclick="submitPopCancel();"/><input type="button" value="'+noCaption+'" onclick="hidePopup(\'popup\',\'popupOverlay\')"/></div>');
	}
...
function disablePageExceptRadioBtn() {
	...
	$('div.container input[type="button"],div.container input[type="submit"],div.container input[type="checkbox"],div.container select').attr('disabled','disabled');
