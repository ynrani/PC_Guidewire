	   ...
	   <table id="search_output_table" class="hoverTable"  style="width:100%; font-size: 13px; border:0; cellpadding:0; cellspacing:1;">
                 ...
                 <c:forEach items="${tdgDtMaskRequestListDTOs}" var="tdgDtMaskRequestListDTOs" varStatus="status">
                   ...
                   <c:choose>
					  <c:when test="${tdgDtMaskRequestListDTOs.status == 'Submitted'}">
					    <td align="center" title="Cancel Request" onmouseover="this.style.cursor='pointer'" onClick="cancelRequest('${tdgDtMaskRequestListDTOs.id}')">
					    <img width="20" height="20" id="close" src="images/3.png"></td>										 
					 </c:when>
					 <c:otherwise>
					   <td align="left"></td>
					  </c:otherwise>
		 ...
		 <ul class="grdPagination">
			                  			<%
											...
											if (currentPage != 1) {
			   							%>
			   									<li><a href="tdmDtMaskDashboard?page=<%= 1 %>">First</a><div>First</div></li>
			   									<li><a href="tdmDtMaskDashboard?page=<%= currentPage-1 %>">&lt; Prev</a><div>&lt; Prev</div>
			   							<%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
			   								 	<li class="disable"><a href="tdmDtMaskDashboard?page=<%= 1 %>">First</a><div>First</div></li>
			   									<li class="disable"><a href="tdmDtMaskDashboard?page=<%= currentPage-1 %>">&lt; Prev</a><div>&lt; Prev</div>
			   							<%
			   								 	}
			   								}
											if(noOfPages > 1) {
										...
										%>
			    		     					<li><a href="tdmDtMaskDashboard?page=<%= currentPage+1 %>">Next &gt;</a><div>Next &gt;</div></li>
			    		     					<li><a href="tdmDtMaskDashboard?page=<%= noOfPages %>">Last</a><div>Last</div></li>
			    		   				<%
											} else {
											    if(noOfPages > 1) {
										%>
			    		     					<li class="disable"><a href="tdmDtMaskDashboard?page=<%= currentPage+1 %>">Next &gt;</a><div>Next &gt;</div></li>
			    		     					<li class="disable"><a href="tdmDtMaskDashboard?page=<%= noOfPages %>">Last</a><div>Last</div></li>
										<%
											    }
											}
										%>
