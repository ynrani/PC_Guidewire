 ...
 function tdgDataMaskingValidationPage3(){
		...
		formObj1.validate({
		     ...
		     submitHandler: function(form) {
		    	 confirmation_dialogYNMR('Submit','Are you sure do you want to submit ?',400,'YesNo','Yes','No');
...
function tdmTestDataReq(){
	var formObj1 = $("#tdmReqTestDataForm");
	var buttonObj = formObj1.find('.btn-cell');
	...
	formObj1.validate({
	     ...
	     submitHandler: function(form) {
	    	 confirmation_dialogTestData('Submit','Are you sure do you want to submit ?',400,'YesNo','Yes','No');
...
function submitPopTestData(){
	 ...
	 $.ajax({
		type:'POST',
			url: "./tdmReqTestData",
			dataType: 'html',
		data: $("#tdmReqTestDataForm").serialize(),
		
			success: function( data ) {
				confirmation_dialogTestdtReq('Success Message','Your Request Id : '+data+' has been submited' ,400,'Ok','','',data);
				$('#submit').removeAttr();
			},
			error:function(jqXHR,error, errorThrown){
