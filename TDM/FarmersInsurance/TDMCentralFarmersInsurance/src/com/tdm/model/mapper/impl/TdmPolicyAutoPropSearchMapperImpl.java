...
public class TdmPolicyAutoPropSearchMapperImpl implements TdmPolicyAutoPropSearchMapper
{
		...
		if (policySummaryStgDOs != null && !policySummaryStgDOs.isEmpty()) {
			...
			for (PolicySummaryStg policySummaryStgDO : policySummaryStgDOs) {
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicydetailId())) {
					tdmPolicyPropertySearchResultDTO.setPolicyDetailId(Long.parseLong(policySummaryStgDO
							.getPolicydetailId()));
				}

				// Policy Number
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicynumber())) {
					tdmPolicyPropertySearchResultDTO.setPolicynumber(policySummaryStgDO.getPolicynumber());
				}

				// Product Type
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicyformcd())) {
					tdmPolicyPropertySearchResultDTO.setProductType(policySummaryStgDO.getPolicyformcd());
				}
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicystatuscd())) {
					if (policySummaryStgDO.getPolicystatuscd().equalsIgnoreCase("issued")
							&& policySummaryStgDO.getTimedpolicystatuscd().equalsIgnoreCase("inForce")) {
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getRiskstatecd())) {
					tdmPolicyPropertySearchResultDTO.setPolicyState(policySummaryStgDO.getRiskstatecd());
				}
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getContracttermtypecd())) {
					if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("AN")) {
						...
						tdmPolicyPropertySearchResultDTO.setPolicyTerm(policySummaryStgDO.getContracttermtypecd());
					}
				}
				...
				if (null != policySummaryStgDO.getCoverageRiskStgs()
					...
					for (int i = 0; i < policySummaryStgDO.getCoverageRiskStgs().size(); i++) {
						if (null != tdmPolicyPropertySearchDTO
								&& StringUtils.isNotEmpty(tdmPolicyPropertySearchDTO.getAddRiskCovge())
								&& tdmPolicyPropertySearchDTO.getAddRiskCovge().equalsIgnoreCase(
										policySummaryStgDO.getCoverageRiskStgs().get(i).getCoveragecd())) {
							tdmPolicyPropertySearchResultDTO.setRiskCovge(policySummaryStgDO.getCoverageRiskStgs()
									.get(i).getCoveragecd());
							break;
						} else {
							tdmPolicyPropertySearchResultDTO.setRiskCovge(policySummaryStgDO.getCoverageRiskStgs()
				...
				if (null != policySummaryStgDO.getEffective()) {
					tdmPolicyPropertySearchResultDTO.setPolicyEffectDt(converDateToString(policySummaryStgDO
							.getEffective()));
				}

				// Expiration date
				if (null != policySummaryStgDO.getExpiration()) {
					tdmPolicyPropertySearchResultDTO.setPolicyExpDt(converDateToString(policySummaryStgDO
							.getExpiration()));
				}
				...
				if (null != reservationDOs && 0 < reservationDOs.size()) {
					for (int j = 0; j < reservationDOs.size(); j++) {
						if (null != reservationDOs.get(j).getReserveDataType()
								&& reservationDOs.get(j).getReserveDataType().equalsIgnoreCase("CSAA_PO")) {
							if (policySummaryStgDO.getPolicynumber().equalsIgnoreCase(
								...
								tdmPolicyPropertySearchResultDTO.setTestCaseId(reservationDOs.get(j).getTestCaseId());
								tdmPolicyPropertySearchResultDTO.setTestCaseName(reservationDOs.get(j)
										.getTestCaseName());
								tdmPolicyPropertySearchResultDTO.setUserId(reservationDOs.get(j).getUserId());
	...
	public List<TdmPolicyAutoSearchResultDTO> converPolicysummaryToTdmPolicyAutoSearchResultDTONew(
		...
		if (policySummaryStgDOs != null && !policySummaryStgDOs.isEmpty()) {
			...
			for (PolicySummaryStg policySummaryStgDO : policySummaryStgDOs) {
				...
				// Policy Detail Id
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicydetailId())) {
					tdmPolicyAutoSearchResultDTO.setPolicyDetailId(Long.parseLong(policySummaryStgDO
							.getPolicydetailId()));
				}

				// Policy Number
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicynumber())) {
					tdmPolicyAutoSearchResultDTO.setPolicynumber(policySummaryStgDO.getPolicynumber());
				}
				// Product Type
				...
				// Risk State
				if (StringUtils.isNotEmpty(policySummaryStgDO.getRiskstatecd())) {
					tdmPolicyAutoSearchResultDTO.setPolicyState(policySummaryStgDO.getRiskstatecd());
				}
				// Policy Term
				if (StringUtils.isNotEmpty(policySummaryStgDO.getContracttermtypecd())) {
					if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("AN")) {
						...
						tdmPolicyAutoSearchResultDTO.setPolicyTerm(policySummaryStgDO.getContracttermtypecd());
					}
				}

				// Policy Coverage
				if (null != policySummaryStgDO.getCoverageStgs() && !policySummaryStgDO.getCoverageStgs().isEmpty()) {
					...
					for (int i = 0; i < policySummaryStgDO.getCoverageStgs().size(); i++) {
						...
						if (null != tdmPolicyAutoSearchDTO
								...
								&& tdmPolicyAutoSearchDTO.getPolicyCovge().equalsIgnoreCase(
										policySummaryStgDO.getCoverageStgs().get(i).getCoveragecd())) {
							tdmPolicyAutoSearchResultDTO.setPolicyCovge(policySummaryStgDO.getCoverageStgs().get(i)
									.getCoveragecd());
					...
					for (int i = 0; i < policySummaryStgDO.getCoverageRiskStgs().size(); i++) {
						if (null != tdmPolicyAutoSearchDTO
								...
								&& tdmPolicyAutoSearchDTO.getRiskCovge().equalsIgnoreCase(
										policySummaryStgDO.getCoverageRiskStgs().get(i).getCoveragecd())) {
							tdmPolicyAutoSearchResultDTO.setRiskCovge(policySummaryStgDO.getCoverageRiskStgs().get(i)
									.getCoveragecd());
							break;
						} else {
							tdmPolicyAutoSearchResultDTO.setRiskCovge(policySummaryStgDO.getCoverageRiskStgs().get(i)
				...
				// Expiration date
				if (null != policySummaryStgDO.getExpiration()) {
					tdmPolicyAutoSearchResultDTO.setPolicyExpDt(converDateToString(policySummaryStgDO.getExpiration()));

				}

				// No of Drivers
				if (null != policySummaryStgDO.getNoOfDriverStg()) {
					tdmPolicyAutoSearchResultDTO.setNoOfDrivers(String.valueOf(policySummaryStgDO.getNoOfDriverStg()
							.getCount()));
				}
				// No Of Name Insu
				...
				if (null != policySummaryStgDO.getNoOfVehiStg()) {
					tdmPolicyAutoSearchResultDTO.setNoOfVehi(String.valueOf(policySummaryStgDO.getNoOfVehiStg()
							.getCount()));
				}

				// No Of Name Vio
				if (null != policySummaryStgDO.getNoOfVioStg()) {
					tdmPolicyAutoSearchResultDTO.setNoOfViola(String.valueOf(policySummaryStgDO.getNoOfVioStg()
							.getCount()));
				}
				...
				if (null != reservationDOs && 0 < reservationDOs.size()) {
					for (int j = 0; j < reservationDOs.size(); j++) {
						if (null != reservationDOs.get(j).getReserveDataType()
								&& reservationDOs.get(j).getReserveDataType().equalsIgnoreCase("CSAA_AU")) {
							if (policySummaryStgDO.getPolicynumber().equalsIgnoreCase(
									reservationDOs.get(j).getReserveDataId())) {
								tdmPolicyAutoSearchResultDTO.setReservedYN(AppConstant.YES);
								tdmPolicyAutoSearchResultDTO.setTestCaseId(reservationDOs.get(j).getTestCaseId());
								tdmPolicyAutoSearchResultDTO.setTestCaseName(reservationDOs.get(j).getTestCaseName());
								tdmPolicyAutoSearchResultDTO.setUserId(reservationDOs.get(j).getUserId());
								Date date1 = reservationDOs.get(j).getUnlockTime();
								long diff = date2.getTime() - date1.getTime();
								switch ((int) (TimeUnit.DAYS.convert(diff, TimeUnit.MILLISECONDS))) {
									case 0:
	...
	public List<TdmReservationDO> converTdmPolicyAutoSearchResultDTOToTdmReservationDO(
			List<TdmPolicyAutoSearchResultDTO> tdmPolicyAutoSearchResultDTOList, String searchcriti, String userId,
			String enviro) {
			...
			for (int i = 0; i < tdmPolicyAutoSearchResultDTOList.size(); i++) {
				...
				if (null != tdmPolicyAutoSearchResultDTO.getReservedYN()) {
					...
					if (null != tdmPolicyAutoSearchResultDTO.getPolicynumber()) {
						reservationDO.setReserveDataId(tdmPolicyAutoSearchResultDTO.getPolicynumber());
					}
	...
	public List<TdmReservationDO> converTdmPolicyPropertySearchDTOToTdmReservationDO(
			...
			String userId, String enviro) {
		logger.info("This is in method TdmPolicyAutoPropSearchMapperImpl.converTdmPolicyPropertySearchDTOToTdmReservationDO() : ");
		List<TdmReservationDO> reservationDOList = null;
		if (null != tdmPolicyPropertySearchResultDTOList && 0 < tdmPolicyPropertySearchResultDTOList.size()) {
			TdmReservationDO reservationDO = null;
			...
			for (int i = 0; i < tdmPolicyPropertySearchResultDTOList.size(); i++) {
				...
				if (null != tdmPolicyPropertySearchResultDTO.getReservedYN()) {
					...
					if (null != tdmPolicyPropertySearchResultDTO.getPolicynumber()) {
						reservationDO.setReserveDataId(tdmPolicyPropertySearchResultDTO.getPolicynumber());
					}
					reservationDO.setLocked("Y");
					reservationDO.setUserId(userId);
					reservationDO.setUnreserve("N");
	...
	public List<TdmPolicyPropertySearchResultDTO> converTdmReservationDOToTdmPolicyPropertySearchDTO(
		...
		if (null != tdmReservationDOlist && 0 < tdmReservationDOlist.size()) {
			...
			for (int i = 0; i < tdmReservationDOlist.size(); i++) {
				...
				tdmPolicyPropertySearchResultDTO.setTestCaseName(tdmReservationDO.getTestCaseName());
	...
	public TdmReqTestDataDO convertdmReqTestDataDTOTotdmReqTestDataDO(TdmReqTestDataDTO tdmReqTestDataDTO) {
		...
		TdmReqTestDataDO tdmReqTestDataDO = null;
		try {

			if (tdmReqTestDataDTO != null) {

				tdmReqTestDataDO = new TdmReqTestDataDO();

				// tdmReqTestDataDO.getReqId();
				tdmReqTestDataDO.setReqDate(new Timestamp(new Date().getTime()));

				tdmReqTestDataDO.setPolicyType(tdmReqTestDataDTO.getPolicyType());
				tdmReqTestDataDO.setPolicyStateCode(tdmReqTestDataDTO.getPolicyStateCode());
				tdmReqTestDataDO.setSrcSys(tdmReqTestDataDTO.getSrcSys());
				tdmReqTestDataDO.setEnvType(tdmReqTestDataDTO.getEnvType());
				tdmReqTestDataDO.setPolicyReqDtl(tdmReqTestDataDTO.getPolicyReqDtl());
				tdmReqTestDataDO.setPolicySts(tdmReqTestDataDTO.getPolicySts());
				tdmReqTestDataDO.setReqTeam(tdmReqTestDataDTO.getReqTeam());
				tdmReqTestDataDO.setReqName(tdmReqTestDataDTO.getReqName());

				tdmReqTestDataDO.setReqSts("Request Submitted");
				tdmReqTestDataDO.setReviewerComm(tdmReqTestDataDTO.getReviewerComm());
				tdmReqTestDataDO.setComments(tdmReqTestDataDTO.getComments());
		...
		return tdmReqTestDataDO;
	}

	@Override
	public List<TdmReqTestDataDTO> convertdmReqTestDataDOsTotdmReqTestDataDTOs(List<TdmReqTestDataDO> tdmReqTestDataDOs) {

		List<TdmReqTestDataDTO> tdmReqTestDataDTOs = null;
		if (null != tdmReqTestDataDOs) {

			TdmReqTestDataDTO tdmReqTestDataDTO = null;
			tdmReqTestDataDTOs = new ArrayList<TdmReqTestDataDTO>();

			for (TdmReqTestDataDO tdmReqTestDataDO : tdmReqTestDataDOs) {

				tdmReqTestDataDTO = new TdmReqTestDataDTO();

				tdmReqTestDataDTO.setReqId(String.valueOf(tdmReqTestDataDO.getReqId()));
				tdmReqTestDataDTO.setReqDate(converDateToString(tdmReqTestDataDO.getReqDate()));

				tdmReqTestDataDTO.setPolicyType(tdmReqTestDataDO.getPolicyType());
				tdmReqTestDataDTO.setPolicyStateCode(tdmReqTestDataDO.getPolicyStateCode());
				tdmReqTestDataDTO.setSrcSys(tdmReqTestDataDO.getSrcSys());
				tdmReqTestDataDTO.setEnvType(tdmReqTestDataDO.getEnvType());
				tdmReqTestDataDTO.setPolicyReqDtl(tdmReqTestDataDO.getPolicyReqDtl());
				tdmReqTestDataDTO.setPolicySts(tdmReqTestDataDO.getPolicySts());
				tdmReqTestDataDTO.setReqTeam(tdmReqTestDataDO.getReqTeam());
				tdmReqTestDataDTO.setReqName(tdmReqTestDataDO.getReqName());

				tdmReqTestDataDTO.setReqSts(tdmReqTestDataDO.getReqSts());
				tdmReqTestDataDTO.setReviewerComm(tdmReqTestDataDO.getReviewerComm());
				tdmReqTestDataDO.setComments(tdmReqTestDataDO.getComments());

				tdmReqTestDataDTOs.add(tdmReqTestDataDTO);
			}
