...
public class TdmPolicyAutoPropSearchDAOImpl implements TdmPolicyAutoPropSearchDAO
{
	...
	public List<PolicySummaryStg> searchPolicyPropRecordsByPolicySearchNew(
			...
			throws DAOException {
		try {
			StringBuffer query = new StringBuffer("SELECT p FROM PolicySummaryStg p WHERE p.lob='HOME'");
			...
			if (StringUtils.isNotEmpty(tdmPolicyPropertySearchDTO.getAddproductType())) {
				query.append(" AND p.policyformcd ='").append(tdmPolicyPropertySearchDTO.getAddproductType())
						.append('\'');
			}
			// Policy Stage
			if (StringUtils.isNotEmpty(tdmPolicyPropertySearchDTO.getPolicyStage())) {
				...
				} else if (tdmPolicyPropertySearchDTO.getPolicyStage().equalsIgnoreCase("Cancelled")) {
			...
			if (StringUtils.isNotEmpty(tdmPolicyPropertySearchDTO.getPolicyState())) {
				query.append(" AND p.riskstatecd ='").append(tdmPolicyPropertySearchDTO.getPolicyState()).append('\'');
			}

			// Policy Term
			if (StringUtils.isNotEmpty(tdmPolicyPropertySearchDTO.getPolicyTerm())) {
				query.append(" AND p.contracttermtypecd ='").append(tdmPolicyPropertySearchDTO.getPolicyTerm())
						.append('\'');
			}
			...
			if (StringUtils.isNotEmpty(tdmPolicyPropertySearchDTO.getAddRiskCovge())) {
				...
				query.replace(query.indexOf("PolicySummaryStg p"), query.indexOf("PolicySummaryStg p")
						+ "PolicySummaryStg p".toString().length(), "PolicySummaryStg p JOIN p.coverageRiskStgs cr");
				query.append(" AND cr.coveragecd ='").append(tdmPolicyPropertySearchDTO.getAddRiskCovge()).append('\'');
			}
			...
			List<PolicySummaryStg> policySummaryStgDOs = managerCsaauser.createQuery(query.toString())
			...
			throw new DAOException(messageSource.getMessage("NRE_ENTITY_MGR_FACTORY_CLOSED_EXCEPTION", null, null),
					illegalStateEx);
		} catch (IllegalArgumentException illegalArgEx) {
			logger.error("MessageConstant.INVALID_QUERY_EXCEPTION" + illegalArgEx);
			throw new DAOException(messageSource.getMessage("INVALID_QUERY_EXCEPTION", null, null), illegalArgEx);
		} catch (NullPointerException nullPointerEx) {
			nullPointerEx.printStackTrace();
			logger.error("MessageConstant.NULL_POINTER_EXCEPTION" + nullPointerEx);
			throw new DAOException(messageSource.getMessage("NULL_POINTER_EXCEPTION", null, null), nullPointerEx);
		} catch (Exception otherEx) {
			logger.error("MessageConstant.DATABASE_EXCEPTION" + otherEx);
			throw new DAOException(messageSource.getMessage("DATABASE_EXCEPTION", null, null), otherEx);
		}
	}
	...
	public List<PolicySummaryStg> searchPolicyAutoRecordsByPolicySearchNew(
		...
		try {
			StringBuffer query = new StringBuffer("SELECT p FROM PolicySummaryStg p WHERE p.lob='AUTO' ");
			...
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getAddproductType())) {
				query.append(" AND p.productcd ='").append(tdmPolicyAutoSearchDTO.getAddproductType()).append("' ");
			}
			...
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getPolicyState())) {
				query.append(" AND p.riskstatecd ='").append(tdmPolicyAutoSearchDTO.getPolicyState()).append("' ");
			}

			// Policy Term
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getPolicyTerm())) {
				query.append(" AND p.contracttermtypecd='").append(tdmPolicyAutoSearchDTO.getPolicyTerm()).append("' ");
			}
			...
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getPolicyCovge())) {
				...
				query.replace(query.indexOf("PolicySummaryStg p"), query.indexOf("PolicySummaryStg p")
						+ "PolicySummaryStg p".toString().length(), "PolicySummaryStg p JOIN p.coverageStgs cs ");

				query.append(" AND cs.coveragecd ='").append(tdmPolicyAutoSearchDTO.getPolicyCovge()).append('\'');
			}
			// Risk Level Coverage
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getRiskCovge())) {
				...
				query.append(" AND cr.coveragecd ='").append(tdmPolicyAutoSearchDTO.getRiskCovge()).append('\'');
			...
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getNoOfDrivers())) {
				query.replace(query.indexOf("PolicySummaryStg p"), query.indexOf("PolicySummaryStg p")
						+ "PolicySummaryStg p".toString().length(), "PolicySummaryStg p JOIN p.noOfDriverStg noOfDr ");
				query.append(" AND noOfDr.count ='").append(tdmPolicyAutoSearchDTO.getNoOfDrivers()).append('\'');
			}

			// No Of Named Insu
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getNoOfNamedInsu())) {
				query.replace(query.indexOf("PolicySummaryStg p"), query.indexOf("PolicySummaryStg p")
						+ "PolicySummaryStg p".toString().length(), "PolicySummaryStg p JOIN p.noOfNameInsuStg noOfNi ");
				query.append(" AND noOfNi.count ='").append(tdmPolicyAutoSearchDTO.getNoOfNamedInsu()).append('\'');
			}

			// No Of Vehi
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getNoOfVehi())) {
				query.replace(query.indexOf("PolicySummaryStg p"), query.indexOf("PolicySummaryStg p")
						+ "PolicySummaryStg p".toString().length(), "PolicySummaryStg p JOIN p.noOfVehiStg noOfVehi ");
				query.append(" AND noOfVehi.count ='").append(tdmPolicyAutoSearchDTO.getNoOfVehi()).append('\'');
			}

			// No Of Vio
			if (StringUtils.isNotEmpty(tdmPolicyAutoSearchDTO.getNoOfViola())) {
				query.replace(query.indexOf("PolicySummaryStg p"), query.indexOf("PolicySummaryStg p")
						+ "PolicySummaryStg p".toString().length(), "PolicySummaryStg p JOIN p.noOfVioStg noOfVio ");
				query.append(" AND noOfVio.count ='").append(tdmPolicyAutoSearchDTO.getNoOfViola()).append('\'');
			}
			...
			if ((elapsedTime / 60000) > 1) {
				throw new DAOException("Query taking more than one min time for validating the records");
			}
	...
	@Override
	public List<TdmReservationDO> getReservedRecords(String userId, String searchType, EntityManager managerCsaaUser)
			throws DAOException {
		try {
			long startTime = System.currentTimeMillis();
			@SuppressWarnings("unchecked")
			List<TdmReservationDO> list = managerCsaaUser.createQuery(
					" FROM TdmReservationDO p where p.reserveDataType IN ('" + searchType + "')").getResultList();
			long stopTime = System.currentTimeMillis();
			long elapsedTime = stopTime - startTime;
			if ((elapsedTime / 60000) > 1) {
				throw new DAOException("Query taking more than one min time for validating the records");
			}
			String str = " FROM TdmReservationDO p where p.reserveDataType IN ('" + searchType + "') AND p.userId'"
					+ userId + "'";
	...
	public String savereqTestData(TdmReqTestDataDO tdmReqTestDataDO, EntityManager managerCsaaUser) throws DAOException {
		...
		try {
			if (null != managerCsaaUser) {
				managerCsaaUser.getTransaction().begin();
				tdmReqTestDataDO = managerCsaaUser.merge(tdmReqTestDataDO);
				managerCsaaUser.getTransaction().commit();
			}
			logger.info(MessageConstant.TDM_DT_MSK_DAO + MessageConstant.TDM_DT_MSK_DAO_GET_SAVED_REQ_DTLS
					+ MessageConstant.LOG_INFO_RETURN);
			return "" + tdmReqTestDataDO.getReqId();
		} catch (IllegalStateException illegalStateEx) {
	...
	public List<TdmReqTestDataDO> getReservedRecords(String userId, int offSet, int recordsperpage,
		...
		try {
			...
			StringBuffer query = new StringBuffer("SELECT P FROM TdmReqTestDataDO P WHERE 0=0 ");

			if (null != userId) {
				query.append("AND P.reqName = '");
				query.append(userId);
				query.append("'");
			}

			@SuppressWarnings(AppConstant.UNCHECKED)
			List<TdmReqTestDataDO> list = managerCsaaUser.createQuery(query.toString()).setFirstResult(offSet)
			...
			logger.error(MessageConstant.TDM_FTD_POLICY_DAO + MessageConstant.TDM_FTD_CLAIM_GET_POL_REC
					+ MessageConstant.LOG_ERROR_EXCEPTION);
			throw new DAOException(MessageConstant.NRE_ENTITY_MGR_FACTORY_CLOSED_EXCEPTION, illegalStateEx);
		} catch (IllegalArgumentException illegalArgEx) {
			logger.error(MessageConstant.TDM_FTD_POLICY_DAO + MessageConstant.TDM_FTD_CLAIM_GET_POL_REC
	...
	public Long allReqTestDataCount(String userId, EntityManager managerCsaaUser) throws DAOException {
		...
		try {
			@SuppressWarnings(AppConstant.UNCHECKED)
			List<TdmReqTestDataDO> list = managerCsaaUser.createQuery(
					"SELECT p FROM TdmReqTestDataDO p where p.reqName = '" + userId + "'").getResultList();
			logger.info(MessageConstant.TDM_FTD_POLICY_DAO + MessageConstant.TDM_FTD_POLICY_DAO_REV_REC
					+ MessageConstant.LOG_INFO_RETURN);
			return (long) list.size();
		} catch (IllegalStateException illegalStateEx) {
			logger.error(MessageConstant.TDM_FTD_POLICY_DAO + MessageConstant.TDM_FTD_POLICY_DAO_REV_REC
					+ MessageConstant.LOG_ERROR_EXCEPTION);
			throw new DAOException(MessageConstant.NRE_ENTITY_MGR_FACTORY_CLOSED_EXCEPTION, illegalStateEx);
		} catch (IllegalArgumentException illegalArgEx) {
			logger.error(MessageConstant.TDM_FTD_POLICY_DAO + MessageConstant.TDM_FTD_POLICY_DAO_REV_REC
	...
	public TdmReqTestDataDO editReqTestData(String userId, EntityManager managerCsaaUser) throws DAOException {
		...
		try {
			...
			StringBuffer query = new StringBuffer("SELECT P FROM TdmReqTestDataDO P WHERE P.reqId = "
					+ Integer.parseInt(userId));

			TdmReqTestDataDO tdmReqTestDataDO = (TdmReqTestDataDO) managerCsaaUser.createQuery(query.toString())
					.getSingleResult();
